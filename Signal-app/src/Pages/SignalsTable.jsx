...
const SignalsTable = () => {
  ...
  const { state: nodeFromState } = location;
  ...
  // Extract assetID from URL params
  const params = new URLSearchParams(location.search);
  const assetId = params.get("assetID"); // Match the parameter name exactly

  console.log("Asset ID from URL:", assetId);

  // Fetch signals when component mounts or assetId changes
  useEffect(() => {
    const fetchSignals = async () => {
      ...
      try {
        console.log("Fetching signals for asset:", assetId);
        const response = await fetch(`https://localhost:7169/api/Signal/${assetId}`);
        ...
        // Update node state if we have data
        if (data.length > 0 && !node) {
          setNode({ id: assetId, signals: data });
        }
      } catch (error) {
      ...
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-100">
        <div className="relative bg-white/90 backdrop-blur-sm border border-blue-200 rounded-3xl shadow-2xl p-10 text-center max-w-lg">
          ...
          <p className="text-gray-600 text-lg mb-8">
            No asset ID was provided in the URL. Please navigate from the asset list.
          </p>
          <button
    ...
    <div className="w-full max-w-7xl mx-auto p-6 bg-white">
      ...
      <div className="mb-8">
        ...
        <p className="text-lg text-gray-600">
          Asset ID: <span className="font-semibold text-gray-800">{assetId}</span>
        </p>
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        {error ? (
          <div className="flex flex-col items-center justify-center h-64 bg-red-50">
            <p className="text-red-700 text-lg font-semibold mb-2">Error Loading Signals</p>
      ...
      {userRole === "Admin" && (
        <SignalOverlay
          show={showOverlay}
          token={storedtoken}
          node={{ id: assetId, name: node?.name || 'Asset' }}
